(function () {
    ...
    const VERSION = "v2.8.1-RG-StrictID-OptimisticPatch"; // MODIFICADO: Versión incrementada
    ...
    function aplicarParcheOptimista(fuente) {
        ...
        if (timeSinceSave < CONSISTENCY_WINDOW_MS && window.lastSavedSnapshot && window.lastSavedSnapshot.length > 0) {
            ...
            fuente.forEach(item => {
                ...
                if (savedItem) {
                    ...
                    if (JSON.stringify(item) !== JSON.stringify(savedItem)) {
                        console.warn(`[Sugerencias RG] ⚠️ CDN desactualizado detectado en ID ${item.id}. Aplicando parche visual local.`);
                        parchesAplicados++;
            ...
            if (parchesAplicados > 0) {
                console.log(`[Sugerencias RG] ✅ Se restauraron ${parchesAplicados} elementos a su estado real para la vista de impresión.`);
            }
        }
        return fuente;
    }
        ...
        const renderCat = (titulo, lista, className) => {
            ...
            lista.forEach(p => {
                ...
                if (esVino) {
                    ...
                    // IMPORTANTE: Forzamos vacío el inglés para ganar espacio
                    htmlNombreEn = "";
                } else {
